 ...
 * 
 * Last Modified: Tuesday, 3rd November 2020 2:16:00 am
...
<template>
    <div style="margin: 0 auto">
        <p style="margin-bottom: 15px">
            <span class="tiny itemText" style="font-weight: bold; background-color: #110101; border-radius: 3px; padding: 5px; margin-bottom: 15px"
            ...
            <span class="itemText" style="background-color: #ad6613; border-radius: 3px; padding: 5px">
                ...
                <span v-if="totalDeltaActive > 0" class="itemText">[&#8593;{{ totalDeltaActive }}]</span>
        ...
        <p style="margin-top: -15px">
            ...
            <span class="itemText" style="background-color: #ad6613; border-radius: 3px; padding: 5px">
                ...
                <span v-if="selStateDeltaActive > 0" class="itemText">[&#8593;{{ selStateDeltaActive }}]</span>
        ...
        methods: {
            renderIndiaMap: function () {
                ...
                if (initWidth > initHeight) {
                    initWidth = initHeight - 255; // Trial & Error
                }

                var svg = d3.select("#india").attr("width", initWidth).attr("height", initHeight);
                ...
                function ready([india]) {
                    ...
                    var projection = d3.geoIdentity().reflectY(true).fitSize([initWidth, initHeight], states);

                    var path = d3.geoPath(projection);

                    d3.select("#states").remove();
                    svg.append("g").attr("id", "states").selectAll("path").data(states.features).enter().append("path").attr("d", path);
                    ...
                    projection = d3.geoIdentity().reflectY(true).fitSize([width, height], states);
                    ...
                    svg.append("g")
                        ...
                        .on("mouseenter", (d) => {
                            ...
                            d3.select(target).attr("fill", "#181818").attr("stroke", "#ff073a").attr("stroke-width", 2);
                            vm.selectedState = d.properties.ST_NM;
                            vm.updateStateData();
                        })
                        .on("mouseleave", (d) => {
                            const target = d3.event.target;
                            d3.select(target).attr("fill", "#000").attr("stroke", "#ff073a5a").attr("stroke-width", 1.5);
                        })
                    ...
                    .then(function (response) {
                        ...
                        for (var i = 0; i < vm.totalData.statewise.length; i++) {
                            if (vm.totalData.statewise[i].state == "Total") {
                                ...
                                vm.mortalityRate = ((vm.totalDeaths / vm.totalConfirmed) * 100).toFixed(2) + "%";

                                vm.totalRecovered = vm.totalData.statewise[i].recovered;
                                vm.totalDeltaRecovered = vm.totalData.statewise[i].deltarecovered;
                                vm.recoveryRate = ((vm.totalRecovered / vm.totalConfirmed) * 100).toFixed(2) + "%";
            ...
            updateStateData: function () {
                ...
                for (var i = 0; i < vm.totalData.statewise.length; i++) {
                    if (vm.totalData.statewise[i].state == vm.selectedState) {
                        ...
                        vm.selStateMortalityRate = ((vm.selStateDeaths / vm.selStateConfirmed) * 100).toFixed(2) + "%";

                        vm.selStateRecovered = vm.totalData.statewise[i].recovered;
                        vm.selStateDeltaRecovered = vm.totalData.statewise[i].deltarecovered;
                        vm.selStateRecoveryRate = ((vm.selStateRecovered / vm.selStateConfirmed) * 100).toFixed(2) + "%";
